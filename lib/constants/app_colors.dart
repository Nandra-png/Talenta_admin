import 'package:flutter/material.dart';

class AppColors {
  // Primary Colors
  static const Color primary = Color(0xFF016B81);
  static const Color secondary = Color(0xFF08B0AC);
  static const Color secondaryLight = Color(0xFFA0DFDF);
  static const Color primaryLight = Color(0xFFE6EAFF);
  static const Color primaryDark = Color(0xFF43508C);

  // Background Colors
  static const Color background = Color(0xFFF9F9F9);
  static const Color backgroundLogin = Color(0xFF016B81);
  static const Color cardBackground = Colors.white;

  // Text Colors
  static const Color textPrimary = Color(0xFF353535);
  static const Color textSecondary = Color(0xFF43508C);
  static const Color textLight = Color(0xFFFEFEFE);
  static const Color textDark = Color(0xFF000000);

  // Status Colors
  static const Color success = Colors.green;
  static const Color error = Color(0xFFFF0000);

  // Shadow Colors
  static Color shadowColor = Colors.grey.withOpacity(0.1);
}
